...
export class SoapHeaderUtil {
  ...
  public createSOAPEnvelopeHeaderSession(requestData: any): object {
    const header = {
      'soapenv:Envelope': {
        '@xmlns:soapenv': 'http://schemas.xmlsoap.org/soap/envelope/',
        '@xmlns:add': 'http://www.w3.org/2005/08/addressing',
          ...
          'oas:Security': {
            'oas:UsernameToken': {
              ...
              'oas:Nonce': {
                '@EncodingType':
                  'http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-soap-message-security-1.0#Base64Binary',
                '#text': this.generateNonce(),
              },
              'oas:Password': {
                '@Type':
                  'http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordDigest',
                '#text': this.generatePasswordDigest(
                  this.generateNonce(),
                  this.getCreatedTimestamp(),
                  process.env.AMADEUS_PASSWORDTEXT || '',
                ),
              },
              'oas1:Created': this.getCreatedTimestamp(),
            },
          },
          AMA_SecurityHostedUser: {
            '@xmlns': 'http://xml.amadeus.com/2010/06/Security_v1',
